...
from config import FUSION, MODELS_DIR, TRAINING
...
def evaluate(model, loader, device) -> dict:
    ...
    mask = labels >= 0
    if mask.any():
        # Same stress formula as config.stress_from_va, in torch.
        stress = ((pa[mask] + 1) / 2) * ((1 - pv[mask]) / 2)
        # 0.35 sits between calm (~0.25) and stressed (~0.7) targets.
        pred = (stress > 0.35).long()
